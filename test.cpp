/*
BROKEN EXAMPLE OF USING FOURIER TRANSFORM ON A 1 DIMENSIONAL SPACE

WORK IN PROGRESS. THIS WILL BE IMPLEMENTED WITH vpaSTRM AT SOME POINT
TO MAKE USE OF THE "COMPUTER VISION" SPACE. A LAME ATTEMPT AT CREATING
SOMETHING SIMILAR TO openCV WITHOUT ALL OF THE DEPENDENCIES. 
...
void FFT(int n, 
        ...
        const double *gIm, 
        double *GRe, 
        double *GIm); //Calculates the DFT (faster)

void plot(int yPos, 
        const double *g, 
        double scale, 
        bool trans, 
        ColorRGB color); //plots a function

void calculateAmp(int n, 
        double *ga, 
        const double *gRe, 
        const double *gIm); //calculates the amplitude of a complex function

int main(int /*argc*/, char */*argv*/[]) {
    screen(640, 480, 0, "Fourier Transform and Filters");

    // primary iterator for loops and conditionals
    int x = 0;
    ...
    while(!endloop) {
        ...
        if (keyPressed(SLDK_c)) {
            for (x; x < n; x++) {
                fRe[x] = 12.5 + p1 * sin(x / p2);
                changed = 1;
            }
        }    
        ...
        if(keyPressed(SDLK_f)) { 
            for(x; x < n; x++) {
                fRe[x] = p1 * (sin(x / p2)+sin(2 * x / p2)) / 2; 
                changed = 1;
            }
        } 
        ...
        if(keyPressed(SDLK_g)) { 
            for(x; x < n; x++) { 
                fRe[x] = 12.5 + p1 *(sin(x / p2) + sin(2 * x / p2)) / 2; 
                changed = 1;
            }
        } 
        ...
        if(changed) {
            ...
            print("Press a-z to choose a function, press the arrows to change \
                    the imaginary part, press space to accept.", 0, 0, RGB_Black);
    ...
    while(!endloop) {
        ...
        if(keyPressed(SDLK_y)) { 
            for(int x = 0; x < n; x++) { 
                FRe2[x] = FRe[x] * sin(x / p2); 
                FIm2[x] = FIm[x] * sin(x / p2);
            } 
            changed = 1;
        } 
